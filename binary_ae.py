...
def get_simple_encoder(latent_dim=3, seq_window=3):
    ...
    #x = tf.clip_by_value(tf.math.round(x), 0, 1)
...
def generate_positive_data_and_labels(
    ...
    #X = X / np.clip(np.max(X.shape[0]), 1, np.inf) - 0.5
...
if __name__ == '__main__':
    ...
    import pandas as pd
    sequence_window = 1
    SUB_SAMPLES = NUM_P_SAMPLES
    TRAINING = True
    NUM_CLUSTERS_VISUAL = 10
    ...
    if TRAINING:
        ...
        encoder.save('models/encoder_bae.hdf5', 'hdf5')
        decoder.save('models/decoder_bae.hdf5', 'hdf5')

    encoder = keras.models.load_model('models/encoder_bae.hdf5', compile=False)

    kmeans = KMeans(n_clusters = NUM_CLUSTERS_VISUAL, random_state = 0).fit(y[:SUB_SAMPLES, :])

    df = pd.DataFrame(data=y[:SUB_SAMPLES, :],
                      columns=['x', 'y', 'z'])

    df['partitions'] = kmeans.labels_[:SUB_SAMPLES].astype(str).reshape(-1, 1)
    df['partitions_int'] = kmeans.labels_[:SUB_SAMPLES]
    df.sort_values(by = ['partitions_int'])
    fig = px.scatter_3d(df, x='x', y='y', z = 'z', color='partitions')
    fig.show()
    fig.write_image('task_space_encoder16.svg')

    sensors = pd.DataFrame(data = X[:SUB_SAMPLES, :],
                           columns = ['c0', 'c1', 'c2', 'c3', 'c4',
                                      'c5', 'c6', 'c7', 'c8', 'c9', 'c10'])
    sensors['partitions'] = kmeans.labels_[:SUB_SAMPLES].astype(str).reshape(-1, 1)
    sensors['timestamp'] = range(sensors.shape[0])
    colors = ['#1f77b4','#ff7f0e','#2ca02c','#d62728','#9467bd','#8c564b','#e377c2',
                    '#7f7f7f','#bcbd22','#17becf']
    fig = go.Figure()
    #for chunk in range(0, SUB_SAMPLES, SUB_SAMPLES // 10):
    #    lower_bound = chunk
    #    upper_bound = chunk + SUB_SAMPLES // 10
    #    slice = sensors.iloc[lower_bound:upper_bound, :]
    #    for part in range(NUM_CLUSTERS_VISUAL):
    #        if slice[slice.partitions == str(part)].first_valid_index() and \
    #            slice[slice.partitions == str(part)].last_valid_index():
    #            fig.add_vrect(
    #                x0=slice[slice.partitions == str(part)].first_valid_index(),
    #                x1=slice[slice.partitions == str(part)].last_valid_index() + 1,
    #                fillcolor=colors[part],
    #                opacity=0.3,
    #                line_width=0,
    #            )

    for i in range(11):
        fig.add_trace(go.Scatter(x=sensors['timestamp'], y=sensors['c' + str(i)],
                                 name='c' + str(i),
                                 line=dict(color='black', width=4)))

    fig.show()
    fig.write_image('sensor_data.svg')
    ...
    df_embedding['partitions_int'] = kmeans.labels_[:SUB_SAMPLES]
    df_embedding.sort_values(by = ['partitions_int'])
    fig = px.scatter(df_embedding, x='dim_0', y='dim_1',
                       color='partitions')
    fig.show()
    fig.write_image('encoder16.svg')
